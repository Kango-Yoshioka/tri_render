...
Camera::Camera()
        : m_EyePoint(Eigen::Vector3d::Zero()),
          ...
          m_zVector(Eigen::Vector3d{0.0, 0.0, 1.0}), m_DistanceToObject(1.0),
          m_FocalLength(0.035), m_ScreenWidth(0.048), m_ScreenHeight(0.027) {
...
void
Camera::lookAt(const Eigen::Vector3d &in_LookAt, const Eigen::Vector3d &in_Up) {
...
void Camera::moveInLocalFrame(const Eigen::Vector3d &in_delta) {
    m_EyePoint += in_delta.x() * m_xVector + in_delta.y() * m_yVector +
                  in_delta.z() * m_zVector;
}
...
void
Camera::rotateCameraInLocalFrameFixLookAt(const double &in_HorizontalAngle) {
    const Eigen::Vector3d lookAtPoint = getLookAtPoint();
